...
for i in ${mountscrypt[@]}; do
  echo -e "$(rclone size $i: --config=${config} --fast-list --user-agent="SomeLegitUserAgent")"  > /config/logs/$i/mountsize-$i.log
done
...
for i in ${mountsuncrypt[@]}; do
  echo -e "$(rclone size $i: --config=${config} --fast-list --exclude="**encrypt**" --user-agent="SomeLegitUserAgent")"  > /config/logs/$i/mountsize-$i.log
done
##sleep for next day##
